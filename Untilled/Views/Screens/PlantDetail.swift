...
struct PlantDetail: View {
    ...
    var plantStatus: [statuses] {
        FetchPlantStatus(
            weatherData: weather ?? WeatherData(temperature: 0, humidity: 0, precipitation: 0, uv: 0),
            plantData: plant
        )
    }
    ...
    var isHealthy: Bool {
        ...
        let currentHumid = weather?.humidity ?? 0
        let currentPrec = weather?.precipitation ?? 0
        let currentUV = Int(weather?.uv ?? 0)
        
        if currentTemp > plant.maxTemperature || currentTemp < plant.minTemperature { return false }
        if currentHumid > plant.maxHumidity || currentHumid < plant.minHumidity { return false }
        if currentPrec > plant.maxPrecipitation || currentPrec < plant.minPrecipitation { return false }
        if currentUV > plant.maxUV || currentUV < plant.minUV { return false }
            ...
            VStack {
                VStack(spacing:20) {
                    ...
                    ZStack {
                        ...
                        ZStack {
                            let animationName = isHealthy ? "Healthy\(plant.plantType)Animation" : "\(plant.plantType)Animation"
                            
                            if let dataAsset = NSDataAsset(name: animationName),
                            ...
                            HStack {
                                ...
                                VStack(alignment: .leading) {
                                    Text("\(weather?.uv ?? 0, specifier: "%.f")UV")
                                        .font(.system(size: 40, weight: .bold))
                                        .foregroundStyle(
                                            Int(weather?.uv ?? 0) > plant.maxUV
                                            ? dangerUp
                                            : (Int(weather?.uv ?? 0) < plant.minUV
                                               ? dangerDown
                                               : .white)
                                        )
